...
def prediction():
    if request.method == 'POST':
        try:
            ...
            try:
                model = load_model(r'D:\Electricc\lstm_models.h5')
            except TypeError as e:
                print(f"Error loading model: {e}")
                return render_template("prediction.html", prediction_text=f'Error loading model: {str(e)}')
...
def historicalgraph():
    return render_template('historicalgraph.html')
